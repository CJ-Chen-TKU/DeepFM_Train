...
# ===== Ensure ML deps at runtime (Torch CPU wheel) =====
import sys, subprocess
try:
    import torch  # noqa
except Exception:
    with st.spinner("Installing PyTorch CPU wheel... this may take a minute on first run"):
...
# Now safe to import the rest of the ML libs
...
def load_table(file_bytes: bytes, filename: str, sheet: str | None):
    ...
    - Excel: choose the proper engine by extension.
      - .xlsx/.xlsm/.xltx/.xltm -> requires openpyxl
      - .xls -> requires xlrd<2.0 (or convert to .xlsx)
    - If sheet is None/empty -> use first sheet (sheet_name=0)
    - If a dict of sheets is returned -> pick the first sheet.
    """
    fname = filename.lower()
    ext = fname.split(".")[-1]
    if ext in ("xlsx", "xlsm", "xltx", "xltm", "xls"):
        sheet_arg = 0 if (sheet is None or str(sheet).strip() == "") else sheet
        engine = "openpyxl" if ext in ("xlsx", "xlsm", "xltx", "xltm") else "xlrd"
        try:
            df = pd.read_excel(io.BytesIO(file_bytes), sheet_name=sheet_arg, engine=engine)
        except ImportError as e:
            if engine == "openpyxl":
                raise ImportError("openpyxl is required for Excel .xlsx files. Install with: pip install openpyxl") from e
            else:
                raise ImportError("xlrd<2.0 is required for legacy .xls files. Install with: pip install 'xlrd<2.0' or convert the file to .xlsx") from e
...
if uploaded and (load_btn or "df" in st.session_state):
    ...
    pattern_default = PRESETS.get(preset, {}).get("pattern", DEFAULT_PATTERN) if target_mode.startswith("Multi-label") else DEFAULT_PATTERN
    if target_mode.startswith("Multi-label"):
    ...
    if start:
        ...
        if target_mode.startswith("Multi-label") and 'label_cols' in locals() and label_cols:
