...
# ===== Ensure ML deps are available (runtime install if missing) =====
import sys, subprocess

def ensure_import(mod_name: str, pip_pkg: str | None = None, extra_args: list[str] | None = None):
    """
    Try to import a module; if it fails, pip install it (optionally with extra args), then import again.
    """
    try:
        __import__(mod_name)
        return
        ...
        if extra_args:
            cmd += extra_args
        cmd += [pip_pkg or mod_name]
        subprocess.check_call(cmd)
        __import__(mod_name)

# Torch (CPU wheel from official index)
ensure_import(
    "torch",
    pip_pkg="torch==2.2.2",
...
# scikit-learn
ensure_import("sklearn", pip_pkg="scikit-learn==1.3.2")

# Optional: iterstrat for multilabel stratified split
try:
    __import__("iterstrat")
    HAS_ITERSTRAT = True
except Exception:
    try:
        subprocess.check_call([sys.executable, "-m", "pip", "install", "iterstrat==0.3.2"])
        HAS_ITERSTRAT = True
    except Exception:
        HAS_ITERSTRAT = False

# ===== Now safe to import ML libs =====
import torch
