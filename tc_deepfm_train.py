...
hide_labels = st.checkbox("Hide label columns in preview", value=True, help="Hide columns matched by the preset pattern (like-p*, like-e*, like-g*).")
...
# Feature suggestion (exclude preset label columns from options)
feature_options_all = [c for c in all_cols if c not in preset_label_cols]
suggested_cat = [c for c in feature_options_all if c != feature_options_all[-1] and is_categorical(df[c])] if feature_options_all else []
suggested_num = [c for c in feature_options_all if c not in suggested_cat and pd.api.types.is_numeric_dtype(df[c])] if feature_options_all else []

st.subheader("🧱 Features")
    with st.expander("Select categorical & numeric features", expanded=True):
        cat_cols = st.multiselect("Categorical columns", options=feature_options_all, default=suggested_cat)
        num_cols = st.multiselect("Numeric columns", options=[c for c in feature_options_all if c not in cat_cols], default=suggested_num)
